...
frappe.ui.form.on("Stock Audit Tool", {
	refresh(frm) {
        ...
        frm.add_custom_button("Create Stock Audit", function () {
            frappe.call({
                method: "jisha_customization.jisha_customization.doctype.stock_audit_tool.stock_audit_tool.create_stock_audit",
                args: {
                    "doc": frm.doc
                },
                freeze: true,
    ...
    fetch_items:function(frm){
        ...
        frappe.call({
            method:"jisha_customization.jisha_customization.doctype.stock_audit_tool.stock_audit_tool.get_items",
            args:{
    ...
    scan_barcode:function(frm){
        if(frm.doc.scan_barcode){
            ...
            frappe.call({
                method:"jisha_customization.jisha_customization.doctype.stock_audit_tool.stock_audit_tool.get_barcode_data",
                args:{
                ...
                callback:function(res){
                    if (res && res.message) {
                        ...
                        if (barcode_data.doctype === "Barcode Entry") {
                            ...
                            // ✅ If no duplicate found, then find suitable row
                            row = items.find(d =>
                                d.item_code === barcode_data.item_code &&
                                frm.doc.warehouse === barcode_data.warehouse
                            );

                            if (!row) {
                                frappe.msgprint(`No matching row found for scanned Item ${barcode_data.item_code}`);
                                frm.set_value("scan_barcode", "");
                                return;
                            }

                            // Check if warehouse_qty > barcode_qty
                            if ((row.warehouse_qty || 0) <= (row.barcode_qty || 0)) {
                                frappe.msgprint(`All barcodes for Item ${barcode_data.item_code} (Row ${row.idx}) have already been scanned. No pending quantity left.`);
                                frm.set_value("scan_barcode", "");
                                return;
                            }

                            let pending_qty = (row.warehouse_qty || 0) - (row.barcode_qty || 0);
                        ...
                        if (barcode_data.doctype === "Box Creation") {
                            ...
                            // ✅ If no duplicate found, find suitable row for the box
                            row = items.find(d =>
                                d.item_code === child_data[0].item_code &&
                                frm.doc.warehouse === child_data[0].warehouse
                            );

                            if (!row) {
                                frappe.msgprint(`No matching row found for scanned Item ${child_data[0].item_code}`);
                                frm.set_value("scan_barcode", "");
                                return;
                            }
                            ...
                            if ((row.warehouse_qty || 0) <= (row.barcode_qty || 0)) {
                                frappe.msgprint(`All barcodes for Item ${child_data[0].item_code} (Row ${row.idx}) have already been scanned. No pending quantity left.`);
                                frm.set_value("scan_barcode", "");
                                return;
                            }
                            ...
                            row.batch = batches.join("\n");
                            row.batch = child_data[0].batch; // set the last batch scanned
